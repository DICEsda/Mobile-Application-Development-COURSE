...
| R8 | Firestore cross-device progress sync | Implemented | `ProgressSyncRepository` integrated into `PlayerViewModel` playback loop. Syncs position, chapter, duration, speed, book title, and per-chapter progress to `users/{uid}/progress/{bookId}`. |
| R9 | Firebase authentication flows | Implemented | `AuthRepository` with `SignInScreen` and `SignUpScreen` in Compose Navigation. Sign out available from Settings. |
| R10 | Biometric library lock | Implemented | `BiometricPrompt` enforced in `MainActivity` when enabled. In-app toggle in Settings persists preference via DataStore. |
| R11 | Notification workflows (daily, streak, milestones) | Implemented | Scheduler + trigger helper + FCM service with per-category toggles in Settings |
...
**Implementation status:** Implemented.
`ProgressSyncRepository` is integrated into `PlayerViewModel`'s playback loop. During playback, progress (position, chapter, duration, speed, book title, and per-chapter progress) is automatically synced to Firestore under `users/{uid}/progress/{bookId}`. On app start, unsynced local progress is pushed to the cloud, and cloud progress can be pulled and merged using timestamp-based conflict resolution.
...
**Implementation status:** Implemented.
When the biometric lock is enabled, `MainActivity` presents `LibraryLockedScreen` on app launch and requires successful `BiometricPrompt` authentication before granting access. The preference is toggled via a "Biometric Lock" switch in the Settings screen, persisted through DataStore via `PreferencesRepository`.
...
|---|---|---|---|---|
| K1 | Notification icons use default system icon | Minor visual inconsistency | Uses generic Android icon | Update with custom app icon |
| K2 | Chapter display formatting varies on small screens | Minor text overflow | Long chapter names may clip | Add ellipsis or adaptive wrapping |
| K3 | Loading indicators use standard Material style | Minor UX polish | Default `CircularProgressIndicator` used | Consider custom branded animations |
| K4 | Error messages surface technical details | UX clarity | Raw exception text shown on some failures | Replace with user-friendly messages |
| K5 | Automated test suite not yet present | Regression risk | Test dependencies configured but no test files | Add unit tests for ViewModels and repositories |
| K6 | Some notification error paths silently catch exceptions | Reduced observability | Silent `catch` blocks in notification services | Add structured logging and retry logic |
